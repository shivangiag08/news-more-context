...
# Initialize the base estimator and the SelfTrainingClassifier with Logistic Regression
base_estimator = LogisticRegression(max_iter=1000)
self_training_model = SelfTrainingClassifier(base_estimator, threshold=0.99, criterion='threshold')
...
# save unlabeled data with labels to disk
unlabeled_data = pd.DataFrame(X_combined[len(global_news_data):])
unlabeled_data['title_sentiment'] = y_combined[len(global_news_data):]
